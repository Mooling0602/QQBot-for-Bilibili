...
| 动态截图 | 可选 | 调用独立截图服务；未配置或调用失败时自动退回文字摘要 |
...
动态监听当前通过 B 站登录账号的 `feed/all` 关注时间线工作，因此账号必须关注目标 UP 主。监听状态保存在 `cache/seen_<mid>.json`；当前首次轮询按时间线逐条建立状态，同一批次中除首条外的已有动态仍可能被识别为新动态。首次启用时建议先设置 `push_dry_run: true` 运行一轮，确认缓存已经建立，再关闭 dry-run 并重启。之后基础轮询间隔为 10 秒，失败时逐步退避，最长 160 秒。
