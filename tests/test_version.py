...
class VersionStatusTests(unittest.TestCase):
    def test_release_image_with_matching_tag_is_stable(self) -> None:
        with (
            patch.object(version, "get_base_version", return_value="0.2.0"),
            patch.object(version, "_git_commit", return_value=None),
            patch.dict(os.environ, {"QQBOT_RELEASE_TAG": "0.2.0"}),
        ):
            self.assertEqual(version.get_status_version(), "v0.2.0 (stable)")
    ...
    def test_release_image_with_missing_or_wrong_tag_is_git(self) -> None:
        with (
            ...
            self.assertEqual(version.get_status_version(), "v0.2.0 (git)")
    ...
    def test_source_checkout_always_includes_the_commit_hash(self) -> None:
        with (
            ...
            self.assertEqual(version.get_status_version(), "v0.2.0 (git: 1da4266)")
